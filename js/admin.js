...
RNKRWP.options = {
	...
	init : function(){
		...
		jQuery( '#rnkrwp_footer-bgcolor' ).miniColors( {
			letterCase	: 'lowercase',
			change		: function( hex, rgb ){

			}
		} );
		jQuery( '#rnkrwp_footer-fontcolor' ).miniColors( {
			letterCase	: 'lowercase',
			change		: function( hex, rgb ){
...
RNKRWP.shortcode = {
	...
	init : function(){
		 ...
		 */
		jQuery( '#rnkrwp_rankerURL' ).bind( 'paste', function(){
			...
			setTimeout( function(){
				...
				if( input !== null && input !== '' ){
					RNKRWP.shortcode.getListData( input );
				}
			}, 10 );
		 ...
		 */
		jQuery( '#rnkrwp_getShortCode' ).click( function(){
	...
	buildCode : function( url, response ){
		...
		var name		= decodeURIComponent( response.name ),
			shortcode	= '[rnkrwp ' + 'id="' + response.id + '" ' + 
				'format="' + ( ( typeof response.listFormatType === 'undefined' || response.listFormatType === '' ) ? 'grid' : response.listFormatType.toLowerCase() ) + '"]';
...
jQuery( document ).ready( function(){
	...
	if( RNKRWP.page === 'rnkrwp-options' ){
		...
		//Bind Events
		/* All rows clicks */
		jQuery( '#rnkrwp_size-rowsall' ).click( function(){
		...
		/* Chicklet color clicks */
		jQuery( '.colorSelect' ).click( function(){
	...
	if( RNKRWP.page === 'rnkrwp-shortcodes' ){
		RNKRWP.shortcode.init();
	}
